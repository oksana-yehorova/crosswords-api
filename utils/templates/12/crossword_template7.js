...
const generateTemplate = async (puzzleState) => {
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 5, 0, ORIENTATION_ENUM.ROW, 'UP_RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 1, 0, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;

    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3, 3, 0, ORIENTATION_ENUM.ROW, 'RIGHT');
  }
  ...
  while (!blockGenerated) {
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3, 8, 2, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 7, 4, 5, ORIENTATION_ENUM.COLUMN, 'DOWN')) return false;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 2, 7, 4, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
