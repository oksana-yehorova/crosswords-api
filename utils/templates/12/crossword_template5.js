...
const generateTemplate = async (puzzleState) => {
  ...
  while (!blockGenerated) {
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 7, 0, 5, ORIENTATION_ENUM.COLUMN, 'LEFT_DOWN');
  }
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 1, 5, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 0, 10, ORIENTATION_ENUM.ROW, 'LEFT_DOWN')) continue;
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 4, 1, 10, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 4, 0, ORIENTATION_ENUM.ROW, 'DOWN_RIGHT')) continue;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 11, 0, ORIENTATION_ENUM.ROW, 'UP_RIGHT')) continue;
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 2, 5, 5, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 8, 7, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3, 4, 11, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
