...
const generateTemplate = async (puzzleState) => {
    ...
    while (!blockGenerated) {
        ...
        if (!await fillRecordOnGrid(tmpPuzzleState, 5, 7, 2, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
        ...
        blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 4, 11, 2, ORIENTATION_ENUM.ROW, 'RIGHT');
    }
    ...
    while (!blockGenerated) {
        ...
        if (!await fillRecordOnGrid(tmpPuzzleState, 4, 6, 7, ORIENTATION_ENUM.COLUMN, 'DOWN')) return false;
        ...
        blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 2, 9, 11, ORIENTATION_ENUM.COLUMN, 'DOWN');
    }
    puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
    blockGenerated = false;
    attempts = 0;
