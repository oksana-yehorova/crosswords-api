...
const generateTemplate = async (puzzleState) => {
  if (!await fillRecordOnGrid(puzzleState, 5, 0, 0, ORIENTATION_ENUM.ROW, 'DOWN_RIGHT')) return false;
  ...
  if (!await fillRecordOnGrid(puzzleState, 4, 1, 1, ORIENTATION_ENUM.COLUMN, 'DOWN')) return false;
  ...
  if (!await fillRecordOnGrid(puzzleState, 4, 0, 5, ORIENTATION_ENUM.ROW, 'LEFT_DOWN')) return false;
  if (!await fillRecordOnGrid(puzzleState, 6, 1, 5, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
  if (!await fillRecordOnGrid(puzzleState, 5, 2, 5, ORIENTATION_ENUM.COLUMN, 'DOWN')) return false;
