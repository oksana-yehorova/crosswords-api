...
const generateTemplate = async (puzzleState) => {
  ...
  while (!blockGenerated) {
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 5, 6, 6, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 7, 8, 4, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3, 7, 4, ORIENTATION_ENUM.ROW, 'RIGHT');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 6, 1, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
    ...
    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3, 6, 0, ORIENTATION_ENUM.ROW, 'UP_RIGHT');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState))
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 0, 5, ORIENTATION_ENUM.COLUMN, 'RIGHT_DOWN')) return false;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 6, 0, 9, ORIENTATION_ENUM.COLUMN, 'DOWN')) continue;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 0, 7, ORIENTATION_ENUM.COLUMN, 'RIGHT_DOWN')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 5, 6, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;

   blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 4, 4, 7, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
