...
const generateTemplate = async (puzzleState) => {
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 9, 4, 2, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 6, 3, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 6, 11, 0, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
