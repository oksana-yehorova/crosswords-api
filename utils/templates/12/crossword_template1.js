...
const generateTemplate = async (puzzleState) => {
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 2, 4, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 6, 6, 5, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
    if (!await fillRecordOnGrid(tmpPuzzleState, 6, 5, 10, ORIENTATION_ENUM.COLUMN, 'DOWN')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 9, 8, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 7, 11, ORIENTATION_ENUM.COLUMN, 'DOWN')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 3, 11, 8, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 7, 9, ORIENTATION_ENUM.COLUMN, 'DOWN')) continue;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 7, 7, ORIENTATION_ENUM.COLUMN, 'DOWN')) continue;

    blockGenerated = await fillRecordOnGrid(tmpPuzzleState, 3,  5, 8, ORIENTATION_ENUM.COLUMN, 'DOWN');
  }
  puzzleState = JSON.parse(JSON.stringify(tmpPuzzleState));
  blockGenerated = false;
  attempts = 0;
  ...
  while (!blockGenerated) {
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 5, 0, ORIENTATION_ENUM.ROW, 'RIGHT')) return false;
    ...
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 11, 2, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 4, 7, 2, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
    if (!await fillRecordOnGrid(tmpPuzzleState, 5, 9, 2, ORIENTATION_ENUM.ROW, 'RIGHT')) continue;
